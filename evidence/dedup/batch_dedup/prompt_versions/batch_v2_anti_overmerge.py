"""Batch dedup prompts — v2 (indexed keys + anti-over-merge).

Changes from v1:
- Entries use numbered indices [1], [2], ... instead of UUIDs — shorter tokens
  and eliminates key truncation issues with weaker models
- Replaced 'MERGE REQUIRES' with 'BEFORE MERGING' verification checklist
  including retrieval-diversity gate and merge group size warning (>3-4 red flag)
- Changed observation calibration from 'prefer merging' to 'when in doubt KEEP'
  with retrieval diversity rationale
"""
...
CALIBRATION NOTE:
When in doubt, KEEP. Each MERGE loses retrieval diversity — the merged
entry appears for one situation text, not many. Over-merging erases the
game-specific context that makes observations retrievable for the right
situations. A genuinely redundant entry will cluster again in the next
cleanup cycle.
