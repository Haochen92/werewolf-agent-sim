...
short games work, long games 422 → fetch-all `get_many` is a paginated table-query whose cost scales
with offset+table). The Langfuse read path is **fiddly** (verified, after two wrong guesses):
`trace.get` TRUNCATES large output (drops `extraction_case` → enumeration-only, NOT the fix); only
`get_many` returns full output but 422s on heavy traces; the `postgame_extraction_` prefix collides
(parent case-span vs per-role child generations). **Fix = local eval-case emission at game-time
