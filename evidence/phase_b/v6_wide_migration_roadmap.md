...
| **Dedup + batch dedup** (`dedup_agent`, `cluster_agent`) | **yes** | rewrite merged situations from the STRUCTURED v6 fields (gate-then-embed), not the prose blob + `SITUATION_STANDARDS` | **already scoped** = `dedup_clustering_and_sp_extraction_plan.md` |
