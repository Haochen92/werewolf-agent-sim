...
| Day-summary judge | `judges/day_summary.py`, `experiments/day_summary_eval.py` (**module-only, no `eval-*` script**) | `extraction/day_summary/experiment_log.md` | ⚠️ **CLAIMED-UNVERIFIED** (apparatus-inspected 2026-06-28) | machine-judge-only, **no golden/human calibration**; leniency-saturated (`village_dynamics`=5.00 flat — zero-info gauge); underpowered (n=18 single-run, "indistinguishable"); **regen+judge confound** (harness regenerates AND judges in one pass → can't isolate judge vs gen noise); **measures intrinsic rubric quality, not the stated objective** (downstream usefulness). = a **smoke test, not a metric** (report.md case 1). |
...
`labeling/{engine,voter,merger,exporter,adapters/*}` + `training/{engine,adapters/*,runner/modal_runner}`. Multi-model voting + CE training (dedup classifier, reranker). **Secondary/parked** (see `feedback-memory-pipeline-prompt-freeze`, `project-fine-tuning-plan`) — not on the headline eval path; map but don't centre the write-up on it.
