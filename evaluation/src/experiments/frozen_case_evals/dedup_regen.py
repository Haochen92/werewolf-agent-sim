"""Re-run dedup decisions on frozen inputs with a different model (CLI).

Reads an existing dedup dataset, replays the dedup prompt through a specified
model (via ``replay.dedup``), and writes a new dataset in the same
``DedupDatasetRecord`` format. The output can be judged directly by ``dedup_judge``.

Usage::

    poetry run python -m evaluation.src.experiments.frozen_case_evals.dedup_regen \\
...
from evaluation.src.replay.dedup import make_dedup_llm, replay_dedup_decision
import time
...
def main() -> None:
    ...
    llm = make_dedup_llm(args.model, args.temperature, **llm_kwargs)
    ...
    for i, record in enumerate(records, 1):
        ...
        result = replay_dedup_decision(llm, case, max_retries=args.max_retries)
