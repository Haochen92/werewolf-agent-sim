"""Discussion-tagger VALIDATION logic (graduated from three frozen evidence scripts).

Instrument-validation module: does the deceiver-side ruler measure what it claims? The thin CLI +
``eval-tagger`` console entry live in ``evaluation.src.cli_runner.discussion_tagger_eval``; this module
holds the reusable load/tag/aggregate logic (the testable seam ``run(cfg)``).
...
to be a separate one-off script under ``evidence/v7_final/``:
  ...
  mode=accuracy — are the per-FIELD tags CORRECT? role_reveal is cross-tabbed against a deterministic
                  ...
                  (deceivers should skew 'manipulative'). Graduated from ``discussion_credit/tagger_accuracy.py``.
  mode=skill    — is the game-level deceiver signal REAL SKILL, not leak or verbosity? Per faction over
                  ...
                  with verbosity partialled out. Graduated from ``runs/v2_full/tagger_skill_retest.py``.
  mode=deleak   — a 2x2 (outcome in/out × all/speakers) ablation that SEPARATES the outcome-leak from
                  ...
                  confounded. Graduated from ``runs/v2_full/tagger_deleak_ablation.py``.
...
# ---------------------------------------------------------------------------
# mode=accuracy  (from evidence/v7_final/discussion_credit/tagger_accuracy.py)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# mode=skill  (from evidence/v7_final/runs/v2_full/tagger_skill_retest.py)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# mode=deleak  (from evidence/v7_final/runs/v2_full/tagger_deleak_ablation.py)
# ---------------------------------------------------------------------------
