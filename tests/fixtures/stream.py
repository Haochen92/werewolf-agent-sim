...
the translator and server-runtime suites). `load_fixture_chunks()` is the plain loader for
module-scope use (e.g. a module-scoped replay fixture); the `fixture_chunks` fixture
re-reads per test, so no test can mutate another's chunks.
"""
